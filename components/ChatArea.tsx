        ...
        <AnimatePresence>
          {(isNearLimit || isOverLimit) && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 5 }}
              className={`absolute left-2 bottom-0 translate-y-1/2 bg-background px-1 text-[10px] font-bold ${
...
export const ChatArea: React.FC<ChatAreaProps> = ({ onRequestPublished, isGuest = false, userId }) => {
  ...
  useEffect(() => {
    // #region agent log
    fetch('http://127.0.0.1:7243/ingest/e4b972a3-10fd-4bed-a97d-4392044af213',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ChatArea.tsx:useEffect',message:'ChatArea mount started',data:{timestamp:Date.now()},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'F'})}).catch(()=>{});
    // #endregion
    ...
    const checkAI = async () => {
      // #region agent log
      fetch('http://127.0.0.1:7243/ingest/e4b972a3-10fd-4bed-a97d-4392044af213',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ChatArea.tsx:checkAI',message:'checkAI starting',data:{startTime},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'F'})}).catch(()=>{});
      // #endregion
      
      const status = await checkAIConnection();
      
      // #region agent log
      fetch('http://127.0.0.1:7243/ingest/e4b972a3-10fd-4bed-a97d-4392044af213',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ChatArea.tsx:checkAI',message:'checkAI completed',data:{duration:Date.now()-startTime,connected:status.connected,error:status.error},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'F'})}).catch(()=>{});
      // #endregion
  ...
  // Only scroll to bottom when AI is connected and there are actual chat messages (not just manual draft)
  useEffect(() => {
    if (aiConnected === true && messages.length > 0) {
      // Check if the last message is not just a manual draft
      const lastMsg = messages[messages.length - 1];
      if (!lastMsg.isDraftPreview || lastMsg.text) {
        scrollToBottom();
      }
    }
  }, [messages, aiConnected]);
  ...
  const handleSend = async (text: string = input) => {
    ...
    try {
      ...
      if (hasPublishedRequest && aiConnected !== false) {
  ...
  const handleStartNewRequest = () => {
    // Simply reset to start a new request (no confirmation needed since button only shows after publishing)
    setMessages([]);
    setInput("");
    clearPendingMedia();
  ...
  return (
    <>
      <div className="flex-1 flex flex-col h-full bg-background relative min-h-0">
        ...
        <div className={`flex-1 overflow-y-auto min-h-0 relative ${messages.length === 0 && aiConnected !== false ? "flex items-center justify-center p-4" : ""}`}>
          ...
          {/* When AI is not connected, still show AI/system messages so errors aren't hidden */}
          {(aiConnected === false
            ? messages.filter((m) => m.isDraftPreview || m.role === "ai")
          ...
          {isLoading && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              className="flex justify-start px-2 md:px-4"
            >
              <div className="flex gap-2">
                <motion.div 
                  className="w-8 h-8 rounded-full bg-gradient-brand flex items-center justify-center shrink-0 shadow-lg"
                  animate={{ scale: [1, 1.1, 1], rotate: [0, 5, -5, 0] }}
                  transition={{ duration: 1, repeat: Infinity }}
                >
                  <span className="text-sm font-black text-white">أ</span>
                </motion.div>
                <div className="bg-gradient-to-r from-secondary to-secondary/50 p-4 rounded-2xl rounded-tr-none border border-border/50 shadow-sm">
         ...
         {isRecording && (
          <div className="absolute inset-x-0 bottom-[110%] p-4 bg-red-500/90 backdrop-blur-md rounded-2xl text-white flex items-center justify-between shadow-2xl animate-in fade-in zoom-in-95">
            ...
            <button
              ...
              className="bg-white text-red-600 px-4 py-1 rounded-full text-[10px] font-black hover:bg-white/90 shrink-0"
            >
              إلغاء
            </button>
          </div>
        ...
        {/* Input bar - only show when AI is connected */}
        {aiConnected !== false && (
        <div className="flex items-center gap-2 rounded-xl p-2 shadow-lg focus-within:shadow-xl focus-within:ring-2 focus-within:ring-primary/30 transition-all relative z-10 mx-3 md:mx-4 mt-2 mb-[calc(env(safe-area-inset-bottom,0px)+32px)] border border-border/50 bg-background/95 backdrop-blur-sm">
          <button
...
const DraftPreviewCard: React.FC<DraftPreviewCardProps> = ({
  ...
  const [smartModeOpen, setSmartModeOpen] = useState(false); // Collapsed by default like drafts sidebar
  const [manualModeOpen, setManualModeOpen] = useState(true);
          ...
          {smartModeOpen && aiConnected === true && (
            ...
            >
              ...
              >
                ...
                <ChevronDown 
                  size={18} 
                  className="text-white rotate-180 transition-transform duration-200"
                />
              </button>
            </motion.div>
          )}
