...
interface FullScreenCardsViewProps {
  requests: Request[];
  myOffers?: Offer[];
  userId?: string;
    ...
    <div className="h-full w-full snap-center snap-always relative flex flex-col bg-black">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 p-6 pb-[calc(env(safe-area-inset-bottom,20px)+80px)]">
        ...
        <motion.button
          whileTap={{ scale: 0.95 }}
          onClick={onClick}
          className="w-full py-4 bg-primary text-white rounded-2xl font-bold text-lg shadow-xl shadow-primary/30 active:shadow-primary/20"
        >
...
// Main Component
export const FullScreenCardsView: React.FC<FullScreenCardsViewProps> = ({
  requests,
  myOffers = [],
  userId,
  onSelectRequest,
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 z-40 flex-col gap-4 top-1/2 -translate-y-1/2">
        <motion.button
...
export default FullScreenCardsView;
