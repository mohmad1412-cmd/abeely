...
export const BottomNavigation: React.FC<BottomNavigationProps> = ({
  ...
  const [dropdownPosition, setDropdownPosition] = useState({ bottom: 0, left: 0, top: 0, right: 0 });
  ...
  const badgeCounts: Record<BottomNavTab | 'profile', number> = {
    ...
    "my-requests": Math.max(0, (unreadMessagesForMyRequests || 0) + (unreadNotificationsForMyRequests || 0)),
    create: 0,
    "my-offers": Math.max(0, (unreadMessagesForMyOffers || 0) + (unreadNotificationsForMyOffers || 0)),
  ...
  const tabs = [
    { id: "marketplace" as BottomNavTab, label: "اكتشف", icon: Search, subtitle: "تصفح طلبات الآخرين وقدم عروضك" },
    { id: "my-requests" as BottomNavTab, label: "طلباتي", icon: FileText, subtitle: "متابعة الطلبات التي أنشأتها" },
    { id: "create" as BottomNavTab, label: "أنشئ طلب", icon: Plus, isCreate: true },
    { id: "my-offers" as BottomNavTab, label: "عروضي", icon: Briefcase, subtitle: "متابعة عروضي التي قدمتها" },
  ...
  return (
    <>
                      ...
                      {isGuest ? (
                        ...
                        <div className="relative flex items-center justify-center">
                          <LogIn 
                            ...
                            fill={isProfileDropdownOpen ? "currentColor" : "none"}
                          />
      ...
      <nav className="hidden md:flex fixed right-0 top-0 bottom-0 z-[60] w-72 bg-card/95 backdrop-blur-xl border-l border-border shadow-lg flex-col py-6">
        <LayoutGroup id="sidebar-nav">
          ...
          <div className="px-4 mb-4">
            ...
            >
              ...
              >
                ...
                >
                  <Plus size={20} strokeWidth={2.5} className="text-white drop-shadow-lg" />
                </motion.div>
                <span className="text-white font-bold text-sm relative z-10">أنشئ طلب</span>
              </motion.div>
            </motion.button>
          </div>
          ...
          <div className="flex-1 flex flex-col gap-1 px-2">
            {tabs.filter(tab => !tab.isCreate && !tab.isProfile).map((tab) => {
              const Icon = tab.icon;
              const isActive = activeTab === tab.id;
              const badgeCount = badgeCounts[tab.id];
      ...
      {isProfileDropdownOpen && !isDesktop && ReactDOM.createPortal(
        <AnimatePresence>
          ...
          >
            ...
            <motion.div
              initial={{ opacity: 0, y: 20, scale: 0.95, x: isDesktop ? 20 : 0 }}
