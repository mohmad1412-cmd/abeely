...
import { generateDraftWithCta } from "../services/aiService.ts";
...
import { uploadRequestAttachments } from "../services/storageService.ts";
...
const extractInfoFromMessage = async (
  ...
  try {
    ...
    const completionPercent = (hasDescription ? 50 : 0) +
      (hasLocation ? 50 : 0);
    ...
    const isFirstInteraction = currentData.description === "" &&
      currentData.location === "";

    // Note: Anthropic API doesn't support audio directly, so audio will be ignored
    if (audioBlob) {
      logger.warn(
        "⚠️ Anthropic API doesn't support audio, audio blob will be ignored",
      );
    }

    const prompt =
      `أنت مساعد ذكي في منصة "أبيلي" للطلبات. تفهم اللهجة السعودية والعربية الفصحى.

## تنبيه مهم: ${
        isFirstInteraction
          ? "هذه أول رسالة من العميل"
          : "هذه رسالة متابعة (لا تكرر الترحيب!)"
      }
...
const simpleExtraction = (message: string): ExtractedData => {
  ...
  if (
    message.includes("عن بعد") || message.includes("اونلاين") ||
    message.includes("أونلاين")
  ) {
  ...
  if (message.includes("عاجل") || message.includes("فوري")) {
    result.deliveryTime = "عاجل";
  }
...
interface CreateRequestV2Props {
  ...
  onPublish: (
    ...
    isEditing?: boolean,
    requestId?: string,
  ) => Promise<string | null>; // Returns request ID on success
...
const AdditionalFieldCard = React.forwardRef<
  HTMLDivElement,
      ...
      >
        ...
        {isGlowing && (
          <motion.div
            ...
            style={{
              background:
                "linear-gradient(45deg, rgba(30,150,140,0.1), rgba(30,150,140,0.2))",
            }}
          />
        )}
      </motion.div>
...
export const CreateRequestV2: React.FC<CreateRequestV2Props> = ({
  ...
  useEffect(() => {
    if (requestToEdit) {
      ...
      const isPreset = ["فوراً", "يوم واحد", "يومين", "أسبوع", "شهر", "غير محدد"]
        .includes(editDeliveryValue);
      if (isPreset) {
  ...
  const restoreFormDataFromGuest = useCallback(() => {
    ...
    try {
      ...
      if (formData.customDeliveryValue) {
        setCustomDeliveryValue(formData.customDeliveryValue);
      }

      // Restore additional fields
      if (
        formData.additionalFields && Array.isArray(formData.additionalFields)
      ) {
  ...
  const handleDescriptionResizeStart = (
    ...
    descriptionResizeStartY.current = "touches" in e
      ? e.touches[0].clientY
      : e.clientY;
    ...
    const handleMove = (moveEvent: MouseEvent | TouchEvent) => {
      ...
      const newHeight = Math.max(
        100,
        Math.min(400, descriptionResizeStartHeight.current + diff),
      );
  ...
  // ==========================================
  const [clarificationPages, setClarificationPages] = useState<
    ClarificationPage[]
  >([]);
      ...
      try {
        ...
        if (draft && !draft.isClarification) {
          ...
          const isTextUnclear = (text: string): boolean => {
            ...
            if (
              unclearKeywords.some((keyword) => textLower.includes(keyword))
            ) return true;
          ...
          if (draft.title) {
            ...
            const isTextUnclear = (text: string): boolean => {
              ...
              const arabicCount = (text.match(arabicChars) || []).length;
              const englishCount = (text.match(englishChars) || []).length;
              const validCharsRatio = (arabicCount + englishCount) /
                text.length;
          ...
          if (draft.categories && draft.categories.length > 0) {
            if (
              draft.categories[0] === "أخرى" || draft.categories[0] === "other"
            ) {
              setShowAdditionalFields(true);
            } else {
              setAdditionalFields((prev) =>
  ...
  const applyFinalReviewData = (review: FinalReview) => {
    ...
    // Category
    if (
      review.system_category && review.system_category !== "غير محدد" &&
      review.system_category !== "أخرى" && review.system_category !== "other"
    ) {
      setAdditionalFields((prev) =>
        prev.map((f) =>
          f.id === "category"
      ...
      !review.system_category || review.system_category === "أخرى" ||
      review.system_category === "other"
    ) {
      setShowAdditionalFields(true);
    }
  ...
  const handlePublishInternal = async (): Promise<
    { success: boolean; requestId?: string | null; error?: string }
  > => {
    ...
    if (attachedFiles.length > 0) {
      try {
        ...
        if (
          ...
          logger.error(
            "Storage bucket missing - request will continue without attachments",
            {
          ...
          uploadError?.message?.includes("صلاحية") ||
    ...
    try {
      ...
      if (
        ...
        errorMessageLower.includes("sql") ||
        errorMessageLower.includes("constraint")
      ) {
        errorMsg =
          "خطأ في قاعدة البيانات. يرجى المحاولة مرة أخرى أو الاتصال بالدعم.";
      }
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 pb-16">
          ...
          {clarificationPages.length > 0 && !showFinalReview && (
            ...
            >
              ...
              <div className="flex items-center justify-center gap-2 mb-6">
                {Array.from({ length: clarificationPages.length }).map((
                  _,
                  index,
                ) => (
                  <motion.div
                  ...
                  >
                    {isAiLoading
                      ...
                      : (
                        <>
                          <span>
                            {currentClarificationPage ===
                                clarificationPages.length - 1
                              ? "إنهاء"
                              : "التالي"}
                          </span>
                          <ChevronLeft size={20} />
                        ...
                        prev.map((f) =>
                          f.id === "category"
                            ? {
                              ...f,
                              enabled: true,
                              value: finalReview.system_category,
                            }
                            : f
                        )
                      );
                      ...
                      try {
                        ...
                        if (result.success && result.requestId) {
                          ...
                          if (onGoToRequest && result.requestId) {
                            setTimeout(
                              () => onGoToRequest(result.requestId as string),
                              2000,
                            );
                          }
                        } else {
                          setIsSubmitting(false);
                          if (
                            result.error && result.error !== "auth_required"
                          ) {
                        ...
                        logger.error(
                          "Error submitting from final review:",
                          error,
                          "service",
                        );
                        setIsSubmitting(false);
        ...
        <div className="space-y-4 mb-6">
          ...
          <motion.div
            ...
            animate={glowingFields.has("description") ||
          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 pt-3 pb-1">
              <span
                className={glowingFields.has("description")
                  ? "text-primary"
            ...
            <div className="px-4 border-t border-border/50">
              ...
              <div className="flex items-center gap-2 pt-3 pb-1">
                <span
                  className={glowingFields.has("location")
                    ? "text-primary"
                    : location
                    ? "text-primary"
                    : "text-muted-foreground"}
                ...
                >
                  الموقع
                  {!location && <span className="text-red-500 mr-1">*</span>}
                  {location && (
                    <motion.span
                      initial={{ scale: 0, opacity: 0 }}
                      animate={{ scale: 1, opacity: 1 }}
            ...
            <div className="px-4 border-t border-border/50">
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    className={attachedFiles.length > 0
                      ? "text-primary"
                      : "text-muted-foreground"}
                  >
                    <Paperclip size={18} />
                          ...
                          {attachedFiles.map((file, index) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div
                                  className="absolute inset-0 flex items-center justify-center bg-black/0 hover:bg-black/20 transition-colors rounded-xl cursor-pointer"
                                  onClick={() => {
                                    setPreviewAttachment({
                                      type: "file",
                                      index,
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <AnimatePresence mode="popLayout">
                  {additionalFields.filter((f) =>
                    f.enabled && f.id !== "category"
                  ).map((field) => (
                    <AdditionalFieldCard
                      key={field.id}
                      field={field}
          ...
          <SubmitButtonWithShake
            ...
            onSubmit={async () => {
              ...
              if (isSubmittingRef.current) {
                logger.warn(
                  "Attempted to submit while already submitting",
                  "service",
                );
                return;
              }
              ...
              try {
                ...
                if (result.success && result.requestId) {
                  ...
                  if (result.error === "auth_required") {
                    ...
                    setSubmitError(
                      "فشل إرسال الطلب. يرجى التحقق من البيانات والمحاولة مرة أخرى.",
                    );
                  }
            ...
            >
              ...
              >
                    ...
                    <motion.button
                      ...
                      onClick={() => {
                        if (previewAttachment.type === "file") {
                          setAttachedFiles((prev) =>
                            prev.filter((_, i) => i !== previewAttachment.index)
                          );
                        }
                ...
                <div className="flex-1 overflow-auto flex items-center justify-center p-4 bg-black/50">
                  {previewAttachment.url.match(
                      /\.(jpg|jpeg|png|gif|webp|svg)$/i,
                    ) ||
                      (previewAttachment.type === "file" &&
                        attachedFiles[previewAttachment.index]?.type.startsWith(
                          "image/",
                        ))
                    ? (
                      <img
                    ...
                    : previewAttachment.url.match(/\.(mp4|webm|ogg)$/i) ||
                        (previewAttachment.type === "file" &&
                          attachedFiles[previewAttachment.index]?.type
                            .startsWith("video/"))
                    ? (
                      <video
                        src={previewAttachment.url}
            ...
            {guestVerificationStep === "phone" && (
              <div className="space-y-4">
                ...
                <p className="text-sm text-muted-foreground text-right">
                  لإتمام إنشاء الطلب، نحتاج للتحقق من رقم جوالك. سيتم إرسال رمز
                  تحقق على رقمك.
                </p>
                <div
                ...
                <div className="flex gap-2">
                  <button
                    onClick={async () => {
                      ...
                      const isTestPhone = cleanPhone.startsWith("0555") ||
                        cleanPhone.startsWith("555");
                      const isValid = isValidSaudiPhone(guestPhone) ||
                        isTestPhone;

                      if (!isValid) {
                        setGuestError(
                          "يرجى إدخال رقم جوال صحيح (9 أو 10 أرقام)",
                        );
                        return;
                      }
                    ...
                    onClick={async () => {
                      ...
                      if (result.success) {
                        ...
                        if (isTestPhone) {
                          ...
                          logger.log(
                            "🔧 DEV MODE: Using guest mode for test phone",
                          );
                          userProfile = {
                            id: localStorage.getItem("dev_test_user_id") ||
                              `test_${Date.now()}`,
                          } as any;
                        } else {
                          ...
                          if (!userProfile?.id) {
                            ...
                            if (!anonId) {
                              setGuestError(
                                "فشل في إنشاء جلسة مؤقتة. حاول مرة أخرى أو سجّل دخول.",
                              );
                              return;
                            }
                            userProfile = await getCurrentUser();
                        ...
                        if (userProfile?.id) {
                          ...
                          if (isTestPhone) {
                            logger.log(
                              "🔧 DEV MODE: Skipping onboarding check for test phone",
                            );
                          ...
                          if (!localOnboarded) {
                            try {
                              ...
                              const hasProfileName = !!profileData?.display_name
                                ?.trim();
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (!isTestPhone) {
                          ...
                          logger.log(
                            "🔧 DEV MODE: Using test phone, allowing request creation without real user",
                          );
                        }
                        ...
                        if (result.success && result.requestId) {
                          ...
                          if (userProfile?.id) {
                            localStorage.removeItem(
                              `abeely_pending_request_form_${userProfile.id}`,
                            );
                          }
                          localStorage.removeItem(
                            "abeely_pending_request_form",
