...
❌ خطأ: نسخ النص حرفياً
- النص المدخل: "سيارة لكزس 2005 صدام"
- العنوان الخطأ: "سيارة لكزس 2005 صدام" (نسخ حرفي)

✅ صحيح: تحويل لصيغة طلب واضحة ومختصرة
- النص المدخل: "سيارة لكزس 2005 صدام"
- العنوان الصحيح: "مطلوب صدام لكزس موديل 2005"
...
❌ خطأ: نسخ النص حرفياً
- النص المدخل: "صيانة مكيف سبليت في الرياض"
- العنوان الخطأ: "صيانة مكيف سبليت في الرياض" (نسخ حرفي)

✅ صحيح: تحويل لصيغة طلب واضحة
- النص المدخل: "صيانة مكيف سبليت في الرياض"
- العنوان الصحيح: "مطلوب فني تكييف سبليت - الرياض"

🎯 قواعد إنشاء العنوان:
1. **ابدأ بكلمة طلبية**: "مطلوب" أو "أبغى" أو "ابحث عن" أو "أحتاج"
2. **لا تنسخ النص المدخل حرفياً**: حوّل النص لعنوان مختصر وواضح
3. **ركز على العناصر الأساسية**: ما المطلوب؟ (مثال: صدام لكزس، تصميم شعار، فني تكييف)
4. **أضف التفاصيل المهمة فقط**: الموديل، النوع، المدينة (إذا كانت مهمة)
5. **اجعل العنوان مختصراً**: 5-10 كلمات كحد أقصى
6. **استخدم صيغة طلب واضحة**: "مطلوب [الشيء المطلوب] [تفاصيل مهمة]"
...
3. إذا كنت متأكداً من التصنيف، ضع التصنيفات في حقل "categories" فقط
4. إذا شككت في التصنيف (الطلب غامض أو قد يناسب أكثر من تصنيف مختلف):
   - ضع التصنيفات المشكوك فيها في حقل "uncertainCategories"
   - ضع "أخرى" في حقل "categories" (سيتم إضافتها تلقائياً)
   - ضع تصنيفاً مقترحاً في حقل "suggestedCategory" (لا تذكر أنه تم تصنيفه في "أخرى")
5. لا تستخدم "غير محدد" - استخدم "أخرى" بدلاً منها
6. التصنيفات يجب أن تكون بالنص العربي الدقيق كما في القائمة
...
أجب بـ JSON فقط:
{
  ...
  "is_ready_to_send": boolean
    ...
    if (mode === "draft") {
      ...
      // إذا كان هناك شك (uncertainCategories موجودة) أو تصنيف مقترح
      if (hasUncertainty || parsed.suggestedCategory) {
        ...
        // لا يوجد شك - نستخدم التصنيفات المؤكدة فقط
