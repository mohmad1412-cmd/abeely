...
const FIXED_CATEGORIES = [
  ...
  // سيارات
  { id: 'car-repair', label: 'صيانة سيارات', keywords: ['سيارة', 'ميكانيكي', 'ورشة', 'صيانة سيارة', 'موتر'] },
...
- إذا كتب "تصميم شعار" فهو يريد من يصمم له شعاراً
- إذا كتب "صيانة مكيف" فهو يريد فني تكييف
- إذا كتب "مدرس رياضيات" فهو يريد معلم خصوصي
- لا تفترض أنه لغز أو سؤال مخفي - النص هو المطلوب بالضبط!

═══════════════════════════════════════════════════════════════

تعليمات مهمة:
1. كن ذكياً، طبيعياً، وعفوياً - تحدث كإنسان حقيقي وليس كروبوت
...
5. الطلب المكتوب = ما يبحث عنه العميل (ليس لغزاً)
...
2. إذا كان الطلب يناسب أكثر من تصنيف، اختر 1-3 تصنيفات الأكثر صلة
3. إذا لم تجد تصنيفاً مناسباً تماماً، اختر "أخرى"
4. لا تستخدم "غير محدد" - استخدم "أخرى" بدلاً منها
  ...
  "is_ready_to_send": boolean
    ...
    if (mode === "draft" && parsed.categories && Array.isArray(parsed.categories)) {
      ...
      if (newCategories.length > 0) {
        ...
        // إضافة "أخرى" إذا لم يكن هناك تصنيفات صالحة كافية
        if (!validCategories.includes("أخرى")) {
      ...
      // إذا لم يكن هناك أي تصنيفات صالحة، نضيف "أخرى"
